...
func createSelectThreatModal(instanceID uuid.UUID, contentContainer ContentContainer, onClose func()) tview.Primitive {
	// Create tabs
	tabs := tview.NewPages()
	tabs.SetBorder(true).SetTitle("Select Threat")

	// Create Select Existing tab
	selectExistingForm := createSelectExistingThreatForm(instanceID, onClose)
